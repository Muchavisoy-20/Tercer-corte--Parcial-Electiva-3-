...
export class ProductosService {
    ...
    async findWithPagination(page: number, limit: number): Promise<{ data: ProductoEntity[], total: number }> {
    ...
    async update(id: number, updateProductDto: Partial<CreateProductDto>): Promise<ProductoEntity> {
        ...
        if (updateProductDto.categoriaId) producto.categoriaId = updateProductDto.categoriaId;
        ...
        if (updateProductDto.stock !== undefined) {
            ...
            if (diff !== 0) {
                await this.logRepository.save({
                    ...
                    motivo: diff > 0 ? 'Entrada de proveedor / reabastecimiento' : 'Ajuste manual por daño / merma',
                    productoId: id
                });
            }
        }
    ...
    async getHealthMetrics() {
        const productos = await this.productoRepository.find({ relations: ['categorias', 'inventario'] });
        const categorias = await this.productoRepository.query('SELECT * FROM categorias');

        const stockCritico = productos.filter(p => (p.inventario?.stock || 0) < (p.inventario?.stockMinimo || 5) && (p.inventario?.stock || 0) > 0);
        const agotados = productos.filter(p => (p.inventario?.stock || 0) === 0);
        const valorTotal = productos.reduce((sum, p) => sum + (Number(p.precio) * (p.inventario?.stock || 0)), 0);
        ...
        productos.forEach(p => {
            ...
            locationStats[loc] = (locationStats[loc] || 0) + (p.inventario?.stock || 0);
        });
